...
/**
 * Manejador para reintento de reverso
...
public class RetryManager {
	...
	/**
	 * Dimension inicial de la coleccion. Fresco que este objeto extiende automaticamente la dimension
	 * cuando se copa la generada aca.
	 */
			...
			while (indexElement <= sizeCollect - 1) {
				if (retryCollect.elementAt(indexElement).getProgramRetry() > 0) {
					...
					if (TargetWork.isAlreadyDone(retryCollect.elementAt(indexElement).getTrazabilityCode())) {
						...
						//Acá se dispara el reverso antes de pasar al siguiente registro del vector de programados
						indexElement++;
					}
				} else {
					synchronized (retryCollect) {
